...
class ApiService {
  ...
  Future<Map<String, dynamic>?> measureGarment({
    ...
    try {
      ...
      final requestBody = {
        'image_url': imageUrl,       // 🔧 スネークケースに修正
        'sku': sku,
        'company_id': effectiveCompanyId,  // 🔧 スネークケースに修正
        'garment_class': garmentClass,     // 🔧 スネークケースに修正
      };
      ...
      ).timeout(const Duration(seconds: 10));  // Worker側は即レスポンス（Prefer:wait削除済み）
  ...
  /// 🆕 GET /api/products/items/:sku - SKU指定で商品実物データを取得
  Future<Map<String, dynamic>?> getProductItemBySku(String sku) async {
    try {
      if (kDebugMode) {
        debugPrint('📡 GET /api/products/items/$sku');
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (kDebugMode) {
          debugPrint('✅ 商品実物データ取得成功');
        }
        return data;
      } else if (response.statusCode == 404) {
        if (kDebugMode) {
          debugPrint('⚠️ 商品実物データが見つかりません (404)');
        }
        return null;
      } else {
        if (kDebugMode) {
