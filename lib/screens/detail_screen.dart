...
class _DetailScreenState extends State<DetailScreen> {
  ...
  // 📏 AI自動採寸結果
  Map<String, dynamic>? _aiMeasurements;
  String? _measurementImageUrl;
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // 📏 AI採寸結果がある場合は表示
            if (_aiMeasurements != null) ...[
              Row(
                children: [
                  Expanded(child: _buildMeasureCard(
                    "着丈", 
                    _aiMeasurements!['body_length']?.toStringAsFixed(1) ?? "68", 
                    true
                  )),
                  SizedBox(width: 12),
                  Expanded(child: _buildMeasureCard(
                    "身幅", 
                    _aiMeasurements!['body_width']?.toStringAsFixed(1) ?? "52", 
                    true
                  )),
                ],
              ),
              ...
              Row(
                children: [
                  Expanded(child: _buildMeasureCard(
                    "肩幅", 
                    _aiMeasurements!['shoulder_width']?.toStringAsFixed(1) ?? "44", 
                    true
                  )),
                  SizedBox(width: 12),
                  Expanded(child: _buildMeasureCard(
                    "袖丈", 
                    _aiMeasurements!['sleeve_length']?.toStringAsFixed(1) ?? "21", 
                    true
                  )),
                ],
              ),
              ...
              // 🖼️ 可視化画像表示
              if (_measurementImageUrl != null && _measurementImageUrl!.isNotEmpty) ...[
                SizedBox(height: 16),
                Text("📐 AI採寸可視化", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                SizedBox(height: 8),
                GestureDetector(
                  onTap: () {
                    // 可視化画像をフルスクリーン表示
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder: (context) => ImagePreviewScreen(
                          imageUrls: [_measurementImageUrl!],
                          initialIndex: 0,
                          heroTag: 'measurement_visualization',
                        ),
                      ),
                    );
                  },
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(16),
                      border: Border.all(color: AppConstants.primaryCyan, width: 2),
                    ),
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(14),
                      child: Image.network(
                        _measurementImageUrl!,
                        fit: BoxFit.contain,
                            ...
                            child: Center(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 8),
                                  Text('画像の読み込みに失敗しました', style: TextStyle(color: Colors.grey[600])),
                                ],
                              ),
                            ),
                          );
              ...
              Row(
                children: [
                  Expanded(child: _buildMeasureCard("着丈", "68", false)),
                  SizedBox(width: 12),
                  Expanded(child: _buildMeasureCard("身幅", "52", false)),
                ],
              ),
              SizedBox(height: 12),
              Row(
  ...
  Future<void> _saveProduct() async {
    try {
      ...
      // 7) ダイアログ閉じて即ダッシュボードに戻る（作業者を待たせない）
      Navigator.pop(context); // プログレスダイアログを閉じる
      
      _showSuccess('✅ 保存完了！${widget.aiMeasureEnabled ? " AI採寸はバックグラウンドで処理中。" : ""}');
      ...
      if (widget.aiMeasureEnabled && allImageUrls.isNotEmpty) {
        ...
        _apiService.measureGarment(
          imageUrl: allImageUrls.first,
          sku: widget.sku.isNotEmpty ? widget.sku : 'NOSKU',
          garmentClass: garmentClass,
        ).then((result) {
          debugPrint('📏 AI採寸レスポンス: ${result?['prediction_id'] ?? 'null'}');
        }).catchError((e) {
      ...
      // 🌐 D1保存（Fire & Forget）
      debugPrint('🌐 D1保存開始（バックグラウンド）');
      _saveToD1WithRetry(
        sku: widget.sku.isNotEmpty ? widget.sku : 'NOSKU',
        imageUrls: allImageUrls,
        newItem: newItem,
      ).then((success) {
        debugPrint('🌐 D1保存完了: ${success ? "成功" : "失敗"}');
      }).catchError((e) {
        debugPrint('⚠️ D1保存エラー: $e');
      });

    } catch (e, stackTrace) {
      // ダイアログが残っている場合は閉じる（安全チェック）
  ...
  Future<bool> _saveToD1WithRetry({
    ...
    for (int retryCount = 0; retryCount < maxRetries; retryCount++) {
      try {
        ...
        // 📏 AI採寸結果はWebhook経由でWorker側がD1に直接保存するため、
        // Flutter側からの保存は不要（Fire & Forget方式）
